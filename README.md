...
### 2. Secret Properties 설정

이 프로젝트는 민감한 정보(API 키, 비밀번호 등)를 별도 파일로 관리합니다.
...
#### 실제 값 입력
`application-secret.properties` 파일을 열고 다음 값들을 입력하세요:

```properties
# OpenAI API Key (필수)
openai.api-key=sk-proj-YOUR_ACTUAL_API_KEY
...
**⚠️ 중요**: `application-secret.properties` 파일은 `.gitignore`에 포함되어 있으므로 Git에 커밋되지 않습니다.
