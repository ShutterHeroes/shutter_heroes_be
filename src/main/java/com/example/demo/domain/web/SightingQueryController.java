...
public class SightingQueryController {
    ...
    @Operation(
        summary = "기준 sighting의 geom을 중심으로 반경 500m 내 목록",
        description = "비로그인/타인: public만, 로그인 사용자: public + 본인 private 포함",
        parameters = {
            @Parameter(name = "sightingId", in = ParameterIn.PATH,
                       required = true, description = "기준 sighting UUID")
        }
    )
    ...
    public List<SightingAroundItemDto> findNearby(
        ...
        return sightingQueryService.findAround(sightingId, viewer);
    }
}
