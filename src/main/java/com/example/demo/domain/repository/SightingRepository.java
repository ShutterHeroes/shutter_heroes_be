...
public interface SightingRepository extends JpaRepository<Sighting, UUID> {
     ...
     * - keyword: species.scientific_name 또는 species.common_name_ko, species.common_name_en로 ILIKE 검색
     * - 본인이 작성한 모든 Sighting 조회 (public/private 모두)
     */
    @Query(value = """
        select
            s.id                                        as id,
        ...
        limit :limit offset :offset
    """, nativeQuery = true)
    List<SightingListRow> findMyReports(
    ...
    /**
     * 내가 제보한 Sighting 전체 개수 조회 (검색 조건 포함)
     */
    @Query(value = """
        select count(*)
        from app.sightings s
